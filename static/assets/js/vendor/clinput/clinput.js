...
clinput.CLInput = class {
    constructor(params) {
        ...
        let labelFrag = "";
        if (label) {
            labelFrag = '<label for="' + this.id + '">' + label + '</label>';
        }
        let initialValue = "";
        if (this.value) {
            initialValue = this.selectedTemplate(this.value);
            this.lastSearchValue = initialValue;
        }
        ...
        input.addEventListener("keydown", (e) => {
            ...
            if (entries.length > 0) {
                this._dispatchForCode(e, arrowPress, entries);
            }
        });
    }
